...
The causes of mortality and determinants of such mortality causes have puzzled researchers for centuries. Despite evidence of the cause of mortality identification inaccuracies {Johns2013}, comprehensive and detailed death certificate records along with more powerful computing and statistical analysis tools have made it much easier to observe actual causes of mortality – at least in the developed world. The determinants of mortality are much harder to pinpoint, with the relative strength of each determinant an even more daunting task.
A popular theory posits that “the wealthier the healthier” – that is, higher incomes directly cause lower mortality, and that higher incomes are the leading cause of lower mortality. Yet this supposed causal relationship can likewise work in reverse in which higher mortality means lower incomes, and studies have shown this explains much of the correlation {Cutler2006}. Surprisingly it has also been observed that recessions seem to improve health outcomes, as people are more likely to do less health-damaging activities (i.e., drinking and smoking) and more health-enhancing activities (i.e., exercise) {Ruhm, 2000}. Another theory suggests that education and ethnicity are the leading causes of lower mortality. For example, according to a study, higher mortality is observed among children of black mothers relative to children of white mothers, regardless of the education level of the mother. However, when comparing only children of white mothers, mothers with higher education levels are observed to show much lower relative mortality for their children {Pamuk1998}. Higher education levels among women are consistently correlated with lower mortality from cardiovascular disease and lung cancer, but also higher mortality from breast cancer {Cutler2006} – demonstrating the importance of comparing mortality by cause. From a long-run perspective, there was hardly any observed difference in life expectancy among England's rich and poor before the Enlightenment period. Yet past this period, there is a widespread general divergence. Since education has always been more accessible for the rich post-Enlightenment, this highlights the strong potential causal link between education (i.e., the development of new ideas of personal and public health such as germ theory of disease, and openness to adjust behaviours or implement policies validating such ideas) and mortality {Cutler2006}. Finally, there is a theory that explains differences in mortality by cause due to differing socio-cultural practices by ethnicity and education levels. For example, lung cancer and cardiovascular diseases should be lower among highly educated people since they are less likely to smoke {Pamuk1998}. While this theory has its merits, the size of its effect is questionable. For example, smoking prevalence is higher among white Americans relative to black Americans, yet this cannot explain black-white differences in American mortality {Cutler2006}.
This overview of mortality causes and the debate on determinants is the motivating context behind this paper. We wish to partially close the gap in understanding between mortality causes and its determinants by analyzing recent data on leading causes of death in New York City, taking a deeper dive into mortality figures by cause and ethnic demographics. We model the leading causes of death by rank and magnitude by ethnicity, hypothesizing that each ethnicity follows a negative binomial distribution. From there, based on relative trends between each ethnicity such as trends in income, education, and health behaviours that supposedly influence mortality, we can infer to a certain extent the validity of existing mortality determinant theories based on the empirical evidence. We find that _______________________________________.
Each section of this paper is organized as follows. Section 2 describes the data and some summary statistics. Section 3 describes our modelling, model hypothesis, and justification. Section 4 presents descriptive statistics and model results. Section 5 is a discussion of the results, including weaknesses, next steps, and conclusive remarks.
...
The dataset comprises several key variables, including: - Year: The year of the recorded death.
...
# Prepare the data
avg_rate_top_causes_by_race <- nyc_data %>%
  mutate(Age_Adjusted_Death_Rate = as.numeric(Age_Adjusted_Death_Rate)) %>%
  group_by(Race.Ethnicity, Leading.Cause) %>%
  summarise(Average_Age_Adjusted_Death_Rate = mean(Age_Adjusted_Death_Rate, na.rm = TRUE), .groups = 'drop') %>%
  arrange(Race.Ethnicity, desc(Average_Age_Adjusted_Death_Rate)) %>%
  mutate(rank = row_number()) %>%
  filter(rank <= 5) %>%
  select(Race.Ethnicity, Leading.Cause, Average_Age_Adjusted_Death_Rate)

# Plotting
gg <- ggplot(avg_rate_top_causes_by_race, aes(x = `Leading.Cause`, y = Average_Age_Adjusted_Death_Rate, fill = `Leading.Cause`)) +
  geom_col() + 
  facet_wrap(~`Race.Ethnicity`, scales = "free_y") + 
  scale_fill_brewer(palette = "Paired") + 
  theme_minimal() + 
...
Contrary to expectations, we find that in modelling the rank and magnitude of the leading cause of death for each ethnicity, utilizing posterior predictive checks, the Poisson distribution offers a better fit relative to the negative binomial distribution (Figure 3; Figure 4; Figure 5; Figure 6). This disproves our hypothesis in which we expected the negative binomial distribution to be a better fit. Results are robust to model cross-validation using LOO for each ethnicity. This is interesting in that a negative binomial distribution is often considered a better fit for estimating mortality, in large part due to overdispersion concerns, yet the Poisson model proved to be a better fit here given the data. 

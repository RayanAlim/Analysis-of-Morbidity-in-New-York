...
kable(model_summary,
      caption = paste("Negative Binomial Regression Summary for", race_name)) %>%
...
# Output the model summary
model_summary_poi <- summary(poisson_model)

# Display the model summary table
kable(model_summary_poi, caption = paste("Poission Summary for", race_name)) %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
```
...
loo_neg_binom_asian <- loo(neg_binom_model)
loo_poisson_asian <- loo(poisson_model)
loo_comparison_asian <- loo_compare(loo_neg_binom_asian, loo_poisson_asian)
print(loo_comparison_asian)
```

```{r}
# Posterior Predictive Checks
# Posterior predictive check for the Negative Binomial model
pp_check_neg_binom_asian <-
  pp_check(neg_binom_model)  + ggtitle("Negative Binomial Model")
ggsave(
  "../data/analysis_data/Model_Output/pp_check_neg_binom_asian.png",
  plot = pp_check_neg_binom_asian,
  width = 10,
  height = 8
)
pp_check_neg_binom_asian
...
pp_check_poisson_asian <-
  pp_check(poisson_model)  + ggtitle("Poisson Model")
ggsave(
  "../data/analysis_data/Model_Output/pp_check_poisson_asian.png",
  plot = pp_check_poisson_asian,
  width = 10,
  height = 8
)
pp_check_poisson_asian
